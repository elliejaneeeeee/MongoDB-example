...
import { Heading, Text, Badge, Flex, Image } from "@chakra-ui/react";
    ...
    const getBookmarks = async () => {
      const bookmarks = await Promise.all(
        bookmarksArray.map(async (bookmark) => {
          if (bookmark.type) {
            const response = await fetch(`/api/${bookmark.type}/${bookmark._id}`);
              ...
              <Link href={bookmark.link} key={bookmark._id} passHref>
                <Flex flexDirection="row" gap="2" padding="2%" borderRadius="5%" bg="#ffe6ee">
                  ...
                  <Flex flexDirection="column">
                    <Badge fontSize="0.7em" bg="#ffc7d2" maxWidth="fit-content">
                      External Article
                    </Badge>
                    <Heading fontSize="md">{bookmark.title}</Heading>

                    <Text fontStyle="italic" fontSize="md" textColor="gray">
                      {bookmark.source}
                    </Text>
                    <Flex>
          ...
          return (
            <Link href={`/forums/${bookmark._id}`} name={"forums"} passHref>
              <Flex flexDirection="column" gap="2">
                <Badge fontSize="0.7em" bg="#ddf2ff" maxWidth="fit-content">
                  Parentify Forums
                </Badge>

                <Heading size="md">{bookmark.title}</Heading>
                <Text fontSize="sm" textColor="gray">
                  {bookmark.author} at {bookmark.date.substring(0, 10)}
                </Text>
